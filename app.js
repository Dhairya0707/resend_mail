document.addEventListener('DOMContentLoaded', () => {
    ...
    const state = {
        ...
        identities: JSON.parse(localStorage.getItem('verified_identities') || '["onboarding@resend.dev"]'),
    ...
    const identitySelectWrapper = document.getElementById('identity-select-wrapper');
    const identityInputWrapper = document.getElementById('identity-input-wrapper');
    ...
    document.getElementById('quick-compose').addEventListener('click', () => switchView('compose'));
    ...
    typeToggles.forEach(btn => {
        btn.addEventListener('click', () => {
            ...
            contentLabel.textContent = state.contentType === 'html' ? 'HTML Content' : 'Plain Text Content';
            htmlContent.placeholder = state.contentType === 'html' ? '<p>Congrats on your first email!</p>' : 'Congrats on your first email!';
            updatePreview();
        });
    });
    ...
    function updatePreview() {
        const content = htmlContent.value || '<p style="color: #71717a; text-align: center; margin-top: 50px;">Start typing to see preview...</p>';
        const doc = previewIframe.contentDocument || previewIframe.contentWindow.document;
        doc.open();
        
        if (state.contentType === 'text') {
            doc.write(`<pre style="font-family: sans-serif; white-space: pre-wrap; padding: 20px; color: #333;">${escapeHTML(htmlContent.value || 'Type something...')}</pre>`);
        } else {
            doc.write(content);
        }
        doc.close();
    ...
    function showToast(message, type = 'success') {
        ...
        setTimeout(() => {
            toast.style.animation = 'slide-up 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275) reverse forwards';
    ...
    function clearHistory() {
        if (confirm('CRITICAL: Are you sure you want to permanently clear all dispatch history? This cannot be undone.')) {
            state.logs = [];
    ...
    window.openLogDetails = function(index) {
        ...
        modalResponse.textContent = JSON.stringify(JSON.parse(log.response), null, 4);

        // Icon color
        modalStatusIcon.style.color = log.status === 'SUCCESS' ? 'var(--accent-green)' : 'var(--error)';
        modalStatusIcon.textContent = log.status === 'SUCCESS' ? 'check_circle' : 'error';
    ...
    function renderIdentities() {
        fromSelect.innerHTML = state.identities.map(email => 
    ...
    saveIdentityBtn.addEventListener('click', () => {
        ...
        if (email && !state.identities.includes(email)) {
            ...
            localStorage.setItem('verified_identities', JSON.stringify(state.identities));
            renderIdentities();
    ...
    emailForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            logToTerminal(`Response received [${response.status}]: ${JSON.stringify(result)}`, response.ok ? 'success' : 'error');
    ...
    function renderLogs() {
        // Full Logs
        fullLogsTableBody.innerHTML = state.logs.map((log, index) => `
        ...
        `).join('') || '<tr><td colspan="5" style="text-align: center; color: var(--text-muted); padding: 2rem;">No dispatches yet.</td></tr>';

        // Dashboard Recent
        recentTableBody.innerHTML = state.logs.slice(0, 5).map((log, index) => {
            ...
            `;
        }).join('') || '<tr><td colspan="4" style="text-align: center; color: var(--text-muted); padding: 2rem;">No activity.</td></tr>';
    }
